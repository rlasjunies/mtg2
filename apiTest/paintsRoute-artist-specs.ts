            ...
            agent.get(paintsUrl)
                ...
                .end((err, resp) => {
                    if (err) {
                        ...
                        //expect(paints).to //mocha.to.be.a("Array");
            ...
            agent.post(paintsUrl).
                ...
                .end((err, resp: sa.Response) => {
                    if (err) {
                        ...
                        expect(doc._id).not.toBeUndefined();//.toBe.not.be.empty;
                        paintsDocID = doc._id;
                        ...
                        delete doc._id;
                        delete doc.__v;
                        expect(doc).toEqual(newPaint);    //.to.deep.equal(newPaint); 
                        done();
                    }
                });
        });
            ...
            agent.get(paintsUrl)
                ...
                .end((err, resp: sa.Response) => {
                    if (err) {
                        ...
                        expect(paints.length).toEqual(paintsInitialLenght + 1);
            ...
            agent.get(paintsUrl + "/" + paintsDocID).
                ...
                .end((err, resp: sa.Response) => {
                    if (err) {
                        ...
                        //mocha expect(paints).to.be.a("Array");
                        expect(paints.length).toEqual(1) //length.to.equal(1);
                        var paint = paints[0];

                        //remove added properties from mongodb
                        delete paint._id;
                        delete paint.__v;
                        //mocha expect(paint).to.deep.equal(newPaint);
                        
                        expect(paint).toEqual(newPaint);
                        done();
                    }
                });
        });
            ...
            agent.get(paintsUrl)
                ...
                .end((err, resp: sa.Response) => {
                    if (err) {
                        ...
                        var paints = <$paintCol.IPaintDocument[]>resp.body;
                        //mocha expect(paints).to.be.a("Array");
                        expect(paints.length).toEqual(paintsInitialLenght);
